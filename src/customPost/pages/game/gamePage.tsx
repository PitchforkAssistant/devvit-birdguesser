    ...
    if (postState.uiDims.width - postState.uiDims.height > 150 && !postState.reduceSize) {
        return (
            <hstack alignment="start top" width="100%" height="100%" gap="none" padding="none">
                ...
                <spacer grow/>
                <vstack alignment="center top" maxWidth="80%" height="100%" gap="none" padding="none">
                    <spacer grow/>
                    {game.image && <GameImage onImagePress={game.expandImagePressed} url={game.image} imageWidth={`${Math.min(postState.uiDims.height * (game.imageAspectRatio * 0.4), postState.uiDims.width * 0.8)}px`} imageHeight={`${postState.uiDims.height * 0.4}px`} cornerRadius="medium" style="metadata" size="xsmall">{game.imageAttribution ?? ""}</GameImage>}
